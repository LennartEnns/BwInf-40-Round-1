# Import der benötigten Module und Anlegen einer Alphabetsliste
import numpy as np
import os
import random
import codecs
...
def abfrage_zahl(frage, fehler, fehler_antwort):
    "Funktion zum Abfragen einer Zahl mit einer bestimmten Fehlerbedingung und -Antwort"
    while True:
        try:
...
# Textdatei und Schwierigkeitsgrad werden abgefragt
leer = "."
...
# Öffnen und Auswerten der ausgewählten Datei
...
class Wort():  # Klasse "Wort" wird definiert
    ...
    def einfügen(self, richtung, anfang):
        "Methode zum Einfügen im Gitter an einer bestimmten Stelle mit bestimmter Ausrichtung"
    ...
    def verbinden(self, index, pos):
        "Methode zum Einfügen, wobei ein Buchstabe mit einem bereits eingefügten Wort geteilt wird"
        ausrichtungen = [0,1]
        if stufe > 2:
    ...
    def posByIndex(self, index):
        "Methode zur Ermittlung der Position eines Buchstabens anhand seines Indexes im Wort"
        return self.pos[index]

    def yx_bereich(self):
        "Methode zur Bestimmung des Bereiches, in dem das Wort platziert werden kann, ohne über den Gitterrand hinauszugehen"
        max = maße.copy()
        min = [0, 0]
...
def verbindungen(wort1, wort2):
    "Listet Indexpaare der zwei übergebenen Worte auf, an denen sie beide denselben Buchstaben haben."
    gleich = []
...
def wörterMitPos(pos):
    "Gibt alle Referenzen von Objekten zurück, von denen ein Buchstabe die angegebene Position hat"
    obj = []
    for i in wörter_obj:
        if pos in i.pos:
            obj.append(i)
...
def verbindenTrue(n):
    "Versucht, das Objekt mit dem angegebenen Index in wörter_obj mit einem Anderen zufällig ausgewählten zu verbinden"
    fertig = False
...
def verbindenFalse(n):
    "Versucht das Objekt mit dem Index n in wörter_obj unabhängig im Gitter an einer zufälligen Stelle einzufügen"
    if stufe > 2:
...
def reset():
    "Setzt das Gitter zurück, löscht alle Instanzen der Klasse Wort und erstellt neue"
    gitter.fill(leer)
...
# Initialisierung der Wort-Objekte
wörter_obj = []
for wort in wörter:
    wörter_obj.append(Wort(True, wort))
random.shuffle(wörter_obj)

# Funktion zum Erhalten der Anzahl aller leeren Stellen im Gitter
def n_leer(): return sum(sum(s == leer for s in z) for z in gitter) 

# Fragmente werden bestimmt
Fragmente = []
if stufe < 4:
    ...
    while len(Fragmente) < F_Anzahl:
        for s in w_strings:
            if random.randint(0, 1):
                ...
                if random.randint(0,2): # Wahrscheinlichkeit = 2/3
...
# Alle Wörter werden jeweils entweder unabhängig oder in Verbindung mit einem anderen Wort eingefügt
gelungen = False
while not gelungen:
    neustart = False
    for n in range(len(wörter_obj)):
        if stufe > 2:
            verbinden = not bool(random.randint(0, 2))  # P=1/3
        else:
            verbinden = bool(random.randint(0, 1))  # P=1/2
        if n > 0:
...
# Lücken werden mit Fragmenten aufgefüllt
f_counter = 0
while n_leer() > 0:
...
# Nicht platzierte Fragmente werden entfernt
rem = []
